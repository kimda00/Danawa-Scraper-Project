...
# 상품명, 가격, 링크, 썸네일 정보 추출
product_elements = soup.find_all("a", class_="click_log_product_standard_title_")
product_prices = soup.find_all("a", class_="click_log_product_standard_price_")
product_images = soup.find_all("a", class_="thumb_link")

for product, price_element, img in zip(product_elements, product_prices, product_images):
    product_name = product.text.strip()
    product_price = price_element.find("strong").text.strip()
